...
| **병렬 워커의 TLS 접근** | 워커 CPU 2.9% (`__tls_init` + `__tls_get_addr`). `tl.*` thread_local을 행마다 접근 → 진입 시 1회 지역 변수로 받으면 제거 가능. 병렬 전용이라 wall 검증이 어려움 |
| 집계 프레임 순회 | 워커 CPU 6.2% (`qdata_evaluate_aggregate_list`). 집계 8개 × 5,800만 행 = 4.6억 회. 줄이려면 행별↔집계별 루프 구조 변경 |
