...
PG는 +0.46s다. 즉 CUBRID의 분석 경로에서 수식 평가는 이미 PG보다 싸다(누계산·peek 적용).
집계 내부식 혼합을 분석까지 확장해도 기대 효과는 0.1s 수준이라 값이 없다.

### 2-c. 분석 격차의 정체 — perf 분해 (2026-08-04)

`SUM(ep) OVER (ORDER BY shipdate)` 3.86s와 `SUM(ep×(1−disc)) OVER (...)` 4.22s를
서버 프로세스에서 프로파일했다. 1.5% 이상 항목의 성격별 합:

| 성격 | 비중 (수식 없음 / 있음) | 대표 심볼 |
|---|---|---|
| **스캔 · deform** | **12.4% / 11.8%** | `heap_attrinfo_read_dbvalues` |
| 힙 순회 · 슬롯 | 약 18% / 약 16% | `heap_next_1page`, `spage_*`, `slot_iterator::next_qualified_slot_with_peek` |
| MVCC 헤더 | 약 5% / 약 6% | `or_mvcc_get_header`, `or_mvcc_get_repid_and_flags` |
| WHERE 필터 평가 | 약 10% / 약 9% | `eval_pred_comp0`, `eval_value_rel_cmp`, `tp_value_compare_with_error` |
| 분석 처리 (직렬 스레드) | 3.05% / 3.02% | `fetch_val_list` |
| **정렬** | **1.5% 컷 위에 없음** | — |
| **분석 함수 · NUMERIC 누적** | **1.5% 컷 위에 없음** | — |

**이 조건에서는 시간이 거의 전부 스캔에서 쓰인다.** 216만 행을 얻기 위해 6,000만 행을
풀스캔하기 때문이다. 정렬도, 분석 함수 자체도, NUMERIC 누적도 상위에 나타나지 않는다.

### 2-d. 스캔을 걷어낸 대조 — 분석 함수 자체는 PG와 대등하다 (결정적)

위 조건은 스캔이 압도해 분석 로직을 볼 수 없다. 그래서 **컬럼 4개·300만 행·필터 없는**
테이블(`t_an`)을 양쪽에 만들어 다시 쟀다. 3회 최솟값.

| 쿼리 | CUBRID (to-be) | PG | 배율 |
|---|---|---|---|
| `SUM(v) OVER (ORDER BY id)` | 2.41s | 2.23s | 1.08x |
| `SUM(v) OVER (PARTITION BY g)` | 1.90s | 1.72s | 1.10x |
| `SUM(v×(1−w)) OVER (ORDER BY id)` | **2.57s** | 2.94s | **0.87x — CUBRID가 빠르다** |

**결론: 스캔 영향을 제거하면 CUBRID 분석은 PG와 대등하고, 수식이 들어가면 오히려 앞선다**
(누계산 + 인자 peek 효과). 따라서 §2-b의 1.65~2.0x는 **전부 스캔·deform 격차**이며,
집계의 V0 격차(2.2x)와 같은 원인 — 이미 "별도 이슈 후보"인 항목이다.
**NUMERIC POC 범위에서 분석을 더 따라잡을 여지는 없다.**

이 조건의 프로파일에서는 리스트파일 I/O 약 19%, 정렬 약 7%, 분석 함수 자체 약 4%가 보인다.
다만 **PG도 같은 일을 하며 비슷한 시간이 걸리므로**(위 표) 이 비중이 곧 비효율이라는 뜻은 아니다.
분석 인자 수식은 체인이 아니라 레거시 이항 경로를 타는데(`float_numeric_db_value_mul` 2.2%
+ `_sub` 1.9%), 그마저 합쳐 0.1s 규모이고 이미 PG보다 빠르므로 확장할 이유가 없다.

**정정 이력**: 이 절을 처음 쓸 때 격차를 "정렬·리스트파일 I/O"로 추론했고, 큰 테이블
프로파일을 보고 "정렬이 아니다"로 정정했다. 최종적으로 정확한 서술은 — 큰 테이블에서는
스캔이 지배해 정렬이 안 보이고, 좁은 테이블에서는 정렬·리스트파일이 보이지만 PG와 대등하다.
어느 조건에서도 **격차의 원인은 스캔**이다.
