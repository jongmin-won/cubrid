...
**결론: 분석 쿼리의 시간은 거의 전부 스캔에서 쓰인다.** 216만 행을 얻기 위해 6,000만 행을
풀스캔하기 때문이다. 정렬도, 분석 함수 자체도, NUMERIC 누적도 상위에 나타나지 않는다.

따라서 **분석의 PG 대비 격차는 집계의 V0 격차(2.2x)와 같은 원인** — 스캔·deform이며,
이는 이미 "별도 이슈 후보"로 분류된 항목이다. **NUMERIC POC 범위에서 분석을 더 따라잡을
여지는 없다.**

**정정**: 이 절을 처음 쓸 때 "남은 격차는 정렬·리스트파일 I/O 쪽"이라고 추론했는데
프로파일이 그것을 반박했다. 정렬은 상위에 없다.
